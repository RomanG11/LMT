...
contract ProxyBasic is Ownable {
    address public impl;
}

contract Proxy is ProxyBasic {
...
contract SalePool is ProxyBasic, TokenWrapper {
...
//   constructor(IERC20 _tokenAddress) public TokenWrapper(_tokenAddress) {
//   }

  

  function init(ERC1155Tradable _nftsAddress, IERC20 _token, ISalePoolUtils _utils) public onlyOwner {
    nfts = _nftsAddress;
    token = _token;
    utils = _utils;
  }
    ...
    emit CardAdded(cardId, mintingAmount);
  }

  function changeCardOpenTime(
        uint cardId,
        uint openTime
    ) public onlyOwner {
        cards[cardId].openTime = openTime;
    }
  
  function changeCardOpenTimeAndBoostedCredits(
        uint cardId,
        uint openTime,
        uint boostedCredits,
        address additionalTokenAddress,
        ...
        cards[cardId].openTime = openTime;
        cards[cardId].boostedCredits = boostedCredits;
        cards[cardId].additionalTokenAddress = additionalTokenAddress;
        cards[cardId].additionalValue = additionalValue;
    }
    ...
    if(hoursPassed < 48) {
        ...
        if(!addValueExists && card.boostedCredits == 0) {
          nextPriceChange = 0;
        }
    ...
    additionalValues = new uint[](len + 1);
    if(hoursPassed < 96 && card.boostedCredits > 0) {
  ...
  function stake(uint256 amount) override public updateReward(msg.sender) {
    ...
    require(endBalance >= utils.getMinStake(), "stake less than minimum");
    require(endBalance <= utils.getMaxStake(), "total stake cannot more than maximum");
    super.stake(amount);
  ...
  function redeemV2(uint256 card, address _paymentTokenAddress) public payable updateReward(msg.sender) {
    ...
    if(paymentMethodExists) {
        if(additionalTokenAddresses[paymentIndex] == address(0) ) {
            ...
            IERC20(_paymentTokenAddress).safeTransferFrom(msg.sender, 0x934dd62782BFe4a8E3f096E014266e5F5adc1b2a, additionalValues[paymentIndex]);
        }
    }
    
    points[msg.sender] = points[msg.sender].sub(totalCredits);
    nfts.mint(msg.sender, card, 1, "");
    
    uint ethValue = address(this).balance;
    if(ethValue > 0) {
        payable(0x934dd62782BFe4a8E3f096E014266e5F5adc1b2a).transfer(ethValue);    
    }
  ...
  function setPoints(address[] calldata _addresses, uint[] calldata _balances) public onlyOwner {
    for(uint i = 0; i < _addresses.length; i++) {
        points[_addresses[i]] = _balances[i];
    }
  }
