...
//UTILS - 0x0A740079dF80b0541EB5f0D6E9DB793f4C5d1C1F
//STAKING-1 - 0x94D7745F5beA39Ac683217C50f49ef47a2758c0B
//IMPL - 0xf51ab819D278942D12e9d7f8E0CfEB8d7Bee37DF
//NFT - 0x73f3da23f8593f4ba50d3b0e3cf5c4c822e1d881
//StakingToken(LMT) -  0x873801ae2ff12d816db9a7b082f5796bec64c82c
//RewardToken(LMT) -  0x873801ae2ff12d816db9a7b082f5796bec64c82c
//feeReceiver - 0x934dd62782bfe4a8e3f096e014266e5f5adc1b2a
//totalReward - 888888000000000000000000 (888888)
...
contract NftStaking is Proxy, IERC1155TokenReceiver {
    ...
    uint public rewardSent;
    uint public totalReward;
    uint public endRewardTime;
    uint public totalRewardPerSecond;
    uint public lastUpdateState;
    // uint public 

    uint public toSendBuffer;
    ...
    modifier updateTotalRewardPerSecond(address _userAddress) {
        
        totalRewardPerSecond -= getUserRewardPerSecond(_userAddress);
        _;
    ...
    function sendApyFixRewards(address[] calldata _users, uint[] calldata _amounts) public onlyOwner {
        for(uint i = 0; i < _users.length; i++) {
            rewardToken.safeTransfer(_users[i], _amounts[i]);
            rewardSent += _amounts[i];
        }
    }

    function apyUpdateFix(address[] calldata _users) public onlyOwner {
        totalRewardPerSecond = 0;
        toSendBuffer = 0;

        for(uint i = 0; i < _users.length; i++) {
            uint reward = getReward(_users[i]);

            lastUserClaim[_users[i]] = block.timestamp;
            
            if(reward > 0) {
                rewardSent += reward;
                rewardToken.safeTransfer(_users[i], reward);
                emit RewardClaimed(_users[i], reward, reward);
            }

            totalRewardPerSecond += getUserRewardPerSecond(_users[i]);
        }

        updateTotalReward();
    }

    function updateTotalReward() internal {
        if(totalReward >= rewardSent.add(toSendBuffer)) {
            if(totalRewardPerSecond != 0) {
                endRewardTime = block.timestamp + ((totalReward.sub(rewardSent).sub(toSendBuffer)) / totalRewardPerSecond);
            } else {
                endRewardTime = block.timestamp;
            }
            lastUpdateState = block.timestamp;
        }
    }
    ...
    function getPoolData(address /*_userAddress*/) public view returns(uint _totalStaked, uint _poolSize, uint _remaining, uint _roiMin, uint _roiMax) {
        ...
        _poolSize = totalReward;
        
        uint extraReward = totalRewardPerSecond * (block.timestamp - lastUpdateState);
        if (extraReward + rewardSent + toSendBuffer < totalReward) {
            _remaining = totalReward - rewardSent - extraReward - toSendBuffer;
        }
    ...
    function getPoolData2(address _userAddress) public view returns(uint _earnedReward, uint _roi, uint _roiBoost, uint _stakedNft, uint _userStakedTokens, uint _freeUnstakeTime) {
        ...
        _freeUnstakeTime = userLastStakeTime[_userAddress].add(unstakeFeeDuration);
    }
    ...
    function updateUser(address _userAddress) public  {
        ...
        toSendBuffer = toSendBuffer.add((block.timestamp - lastUpdateState) * totalRewardPerSecond);
        if(userReward > 0) {
    
            toSendBuffer = toSendBuffer.sub(userReward);

            rewardSent += userReward;
    ...
    function init(
        ...
        unstakeFeeDuration = 3 days;
        unstakeFee = 0.01e18; //1%
    ...
    function setApyStruct() public onlyOwner {
    ...
    function stakeCards(uint[] calldata _ids) public updateState(msg.sender) updateTotalRewardPerSecond(msg.sender) {
        uint totalPrice;
    ...
    function withdraw(uint[] calldata _ids) public updateState(msg.sender) updateTotalRewardPerSecond(msg.sender) {
        uint totalPrice;
        ...
        if(block.timestamp < userLastStakeTime[msg.sender].add(unstakeFeeDuration) 
            || block.timestamp >= endRewardTime) //You do not pay unstaking fee when staking event is over
        {
            //charge fee
        ...
        require(tokensStaked[msg.sender] >= apyStruct[0].amount || (higherTierCardsStaked[msg.sender].length == 0 && lowerTierCardsStaked[msg.sender].length == 0), "total stake less than minimum");
    ...
    function getApyByStake(uint _amount) public view returns(uint) {
        ...
        for(uint i = 1; i < apyStruct.length; i++) {
            if(_amount < apyStruct[i].amount) {
                return apyStruct[i-1].apy;
            }
        }
    ...
    function getApy(uint[] calldata _nfts) public view returns(uint apy, uint boostedApy) {
        uint totalPrice;
        for(uint i = 0; i < _nfts.length; i++) {
            bool isHigherTier;
            uint price;

            uint higherTierPrice = utils.higherTierCards(_nfts[i]);
            if(higherTierPrice > 0) {
                isHigherTier = true;
                price = higherTierPrice;
                uint boosted = utils.goatCards(_nfts[i]);
                if(boosted > 0) {
                    boostedApy = boosted;
                }
            } else {
                uint lowerTierPrice = utils.lowerTierCards(_nfts[i]);
                require(lowerTierPrice != 0, "card is not stakable");
                price = lowerTierPrice;
            }
        ...
        uint baseAPY = getApyByStake(totalPrice);
        apy = baseAPY * (1e18 + boostedApy) / 1e18;
    }
    ...
    function getReward(address _userAddress) public view returns(uint) {
        ...
        uint finalTime = endRewardTime < block.timestamp ? endRewardTime : block.timestamp;
        return (finalTime - lastUserClaim[_userAddress]) * getUserRewardPerSecond(_userAddress);
        // return (block.timestamp - lastUserClaim[_userAddress]);
    }
    
    function getUserRewardPerSecond(address _userAddress) public view returns(uint) {
        return tokensStaked[_userAddress] * getTotalApy(_userAddress) / 1e18 / (60*60*24*30);
    }
