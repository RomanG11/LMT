...
contract SalePool is TokenWrapper, Ownable {
  ...
  uint public constant MIN_STAKE = 10e18;
  ...
  constructor(ERC1155Tradable _nftsAddress, IERC20 _tokenAddress) public TokenWrapper(_tokenAddress) {
    nfts = _nftsAddress;
  }
  ...
  function earned(address account) public view returns (uint256) {
    ...
    return points[account].add(blockTime.sub(lastUpdateTime[account]).mul(getProduction(balanceOf(account))));
  }

  function stake(uint256 amount) override public updateReward(msg.sender) {
    require(amount >= MIN_STAKE, "min stake - 10 LMT");
    super.stake(amount);
